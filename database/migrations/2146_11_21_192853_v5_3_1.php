...
class V531 extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('referral_code_usages', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('used_by')->nullable(); // Correct : nullable() pour rendre la colonne nullable
            $table->foreign('used_by')->references('id')->on('users')->onDelete('cascade');
            $table->string('referral_code')->nullable();
            $table->index('used_by')->nullable(); // Pas besoin de nullable() ici, les index ne sont pas affectés par la nullabilité
            $table->index('referral_code')->nullable();
            $table->timestamps();
        });
