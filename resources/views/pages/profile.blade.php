        ...
        <div class="min-vh-100 col-12 col-md-8 border-right pr-md-0">
            ...
            <div class="container pt-2 pl-0 pr-0">
                ...
                <div class="pt-2 pl-4 pr-4">
                    <h5 class="text-bold d-flex align-items-center">
                        ...
                        @php
                            ...
                            $lastSeen = $lastActive
                                ? $lastActive->setTimezone('Europe/Paris')->diffForHumans(['parts' => 2])
                                : 'jamais';
                        @endphp
                            ...
                            <div class="d-flex align-items-center">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="position-relative mr-2">
                                        <span
                                            class="position-absolute rounded-circle {{ $isOnline ? 'bg-success' : 'bg-danger' }}"
                                            style="width: 10px; height: 10px; margin: -5px 0px 0px 7px; opacity: 0.75; animation: ping 1.5s infinite;">
                                        </span>
                                        <span class="rounded-circle {{ $isOnline ? 'bg-success' : 'bg-danger' }}"
                                            style="width: 10px; height: 10px;">
                                        </span>
                                    </div>
                                    <span class="small {{ $isOnline ? 'text-success' : 'text-danger' }}"
                                        style="margin: -2px 0px 0px 19px;">
                                        {{ $isOnline ? __('En ligne') : __('Hors ligne') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <h6 class="text-muted"><span class="text-bold"><span>@</span>{{ $user->username }}</span>
                        {{-- - Last seen X time ago --}}</h6>
                </div>

                <div class="pt-2 pb-2 pl-4 pr-4 profile-description-holder">
                    <div
                        class="description-content {{ $user->bio && !getSetting('profiles.disable_profile_bio_excerpt') ? 'line-clamp-3' : '' }}">
                        @if ($user->bio)
                    ...
                    @if ($user->bio && !getSetting('profiles.disable_profile_bio_excerpt'))
                        <span class="text-primary pointer-cursor show-more-actions d-none"
                            onclick="Profile.toggleFullDescription()">
                    ...
                    @if ($user->userCountry)
                        <div class="d-flex align-items-center mr-2 text-truncate mb-0 mb-md-0">
                            ...
                            <div class="text-truncate ml-1">
                                ...
                                @if ($user->userCountry->flag)
                                    <img src="{{ $user->userCountry->flag }}" alt="{{ $user->userCountry->name }}"
                                        class="ml-1" style="height: 15px;">
                                @endif
                            </div>
                        </div>
                    @endif
                ...
                <div class="referral-section p-3 border rounded">
                    @if (auth()->user())
                        <p class="mb-2">
                            <strong>{{ __('Votre lien de parrainage :') }}</strong>
                            <input type="text" value="{{ url('/register?ref=' . auth()->user()->referral_code) }}"
                                readonly class="form-control pointer-cursor" data-toggle="tooltip" data-placement="top"
                                title="{{ __('Cliquez pour copier') }}"
                                onclick="navigator.clipboard.writeText(this.value); alert('{{ __('Lien copié !') }}');">
                        </p>
                        <p class="mb-2">
                            <span class="text-muted">{{ __('Nombre de parrainages :') }}</span>
                            <strong>{{ $totalUsedBy ?? 'Non utilisé' }}</strong>
                        </p>
                        @if (auth()->user()->has_referral_discount)
                            <div class="alert alert-success">
                                <i class="fas fa-check-circle"></i>
                                {{ __('Félicitations ! Vous avez obtenu votre réduction de 50% !') }}
                            </div>
                        @endif
                    @endif
                </div>
                    ...
                    @if ((!Auth::check() || Auth::user()->id !== $user->id) && !$hasSub)
                        <div class="p-4 subscription-holder">
                            ...
                            @if (count($offer) && $offer['discountAmount']['30'] > 0)
                                ...
                                <small
                                    class="">{{ __('Offer ends label', ['date' => $offer['expiresAt']->format('d M')]) }}</small>
                            @endif
                            @if ($hasSub)
                            ...
                            @else
                                ...
                                <div class="d-flex justify-content-between">
                                    @if ($user->profile_access_price_6_months || $user->profile_access_price_12_months)
                                        <small>
                                            <div class="pointer-cursor d-flex align-items-center"
                                                onclick="Profile.toggleBundles()">
                                    ...
                                    @if (count($offer) && $offer['discountAmount']['30'] > 0)
                                        <small
                                            class="">{{ __('Regular price label', ['currency' => getSetting('payments.currency_code') ?? 'USD', 'amount' => $user->offer->old_profile_access_price]) }}</small>
                                    @endif
                                </div>
                ...
                <div class="mt-3 inline-border-tabs">
                    <nav class="nav nav-pills nav-justified text-bold">
                        <a class="nav-item nav-link {{ $activeFilter == false ? 'active' : '' }}"
                            href="{{ route('profile', ['username' => $user->username]) }}">{{ trans_choice('posts', $posts->total(), ['number' => $posts->total()]) }}
                        </a>
